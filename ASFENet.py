...
class Network:
    ...
    def inference(self, inputs, is_training):
        ...
        for i in range(self.config.num_layers):
            f_encoder_i = self.dilated_res_block(
                ...
                inputs["xyz"][i],
                inputs["neigh_idx"][i],
                d_out[i],  # d_out=[16, 64, 128, 256, 512]
                "Encoder_layer_" + str(i),
                is_training,
            )
            f_encoder_i = self.adaptive_features_scaler(f_encoder_i,'Encoder_layer_2_'+str(i),is_training)
        ...
        for j in range(self.config.num_layers):
            ...
            # print("Shape of f_encoder_list[-{}]:{}".format(j + 2, f_encoder_list[-j - 2].shape))
            # print("Shape of f_interp_i:{}".format(f_interp_i.shape))
    ...
    def building_block(self, xyz, feature, neigh_idx, d_out, name, is_training):
        ...
        f_xyz = self.Spatial_Structur_Encoder(xyz, neigh_idx)
    ...
    # adaptive_features_scaler
    def adaptive_features_scaler(self, feature, name, is_training):
        # (B,N,1,d)
        d = feature.get_shape()[3].value
        f_mlp1 = helper_tf_util.conv2d(feature, 2 * d, [1, 1], name + 'mlp1', [1, 1], 'VALID', True, is_training)
        f_mean = tf.reduce_mean(f_mlp1, axis=1, keepdims=True)  # (B,1,1,d)
        f_max = tf.reduce_max(f_mlp1, axis=1, keepdims=True)
        f_sum = tf.concat([f_mean, f_max], axis=3)
        f_mlp2 = helper_tf_util.conv2d(f_sum, 4 * d, [1, 1], name + 'mlp2', [1, 1], 'VALID', True, is_training)
        f_mlp3 = helper_tf_util.conv2d(f_mlp2, d, [1, 1], name + 'mlp3', [1, 1], 'VALID', True, is_training)

        scale_weights = tf.nn.sigmoid(f_mlp3)  # (B,1,1,d)

        # Scaling broadcasting
        scaled_feature = tf.multiply(feature, scale_weights)  # element-wise multiplication with broadcasting

        return scaled_feature

    def Spatial_Structur_Encoder(self, xyz, neigh_idx):
        ...
        # relative angle
        theta = tf.atan2(relative_xyz[:, :, :, 1], relative_xyz[:, :, :, 0])
        phi = tf.atan2(relative_xyz[:, :, :, 2],
                       tf.sqrt(tf.square(relative_xyz[:, :, :, 0]) + tf.square(relative_xyz[:, :, :, 1])))
        ...
        relative_feature = tf.concat(
            [relative_dis, relative_xyz, theta[:, :, :, tf.newaxis], phi[:, :, :, tf.newaxis], xyz_tile, neighbor_xyz],
            axis=-1
        )
    ...
    def att_pooling(feature_set, d_out, name, is_training):
        ...
        d = feature_set.get_shape()[3].value  # d：
        f_reshaped = tf.reshape(feature_set, shape=[-1, num_neigh, d])  # (B*N,k,d)
        att_tunnel = tf.reduce_mean(f_reshaped, axis=1,keepdims=True)  # (B*N,1,d)
        f_att_tunnel1 = tf.layers.dense(
            att_tunnel, d, activation=None, use_bias=False, name=name + "fc1"
        )
        f_att_tunnel2 = tf.layers.dense(
            f_att_tunnel1, d, activation='relu', use_bias=False, name=name + "fc2"
        )
        att_tunnel = tf.nn.sigmoid(f_att_tunnel2)  # (B*N,1,d)
        att_tunnel = tf.tile(att_tunnel,[1,num_neigh,1])
        att_activation = tf.layers.dense(
            f_reshaped, d, activation=None, use_bias=False, name=name + "fc3"
        ) + att_tunnel
