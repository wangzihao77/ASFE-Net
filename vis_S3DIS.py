...
basedir="/data/S3DIS/input_0.040"  # for label

# vis_list=os.listdir(basedir)
vis_list=['Area_5_hallway_1.ply']
plot_colors = Plot.random_colors(13, seed=2) #13 CLASS
Flag=True
for name in vis_list:
    print(basedir+name)
    # data = read_ply(basedir+name)
    data = read_ply("/data/S3DIS/input_0.040/Area_5_hallway_1.ply")
    if Flag:
        PID = os.fork()
        Flag=False
    if PID == 0:
        ...
        Plot.draw_pc_sem_ins(pc_xyz, pc_sem_ins)
