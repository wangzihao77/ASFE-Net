...
class ConfigToronto3D:
    ...
    max_epoch = 100  # maximum epoch during train
...
class ConfigS3DIS:
    ...
    sub_sampling_ratio = [4, 4, 4, 4, 2]  # sampling ratio of random sampling at each layer
    d_out = [16, 64, 128, 256, 512]  # feature dimension
    #d_out = [8, 32, 64, 128, 256]

    #d_out = [8, 32, 128, 256, 512]
    noise_init = 3.5  # noise initial parameter
    max_epoch = 100  # maximum epoch during training
    learning_rate = 1e-2  # initial learning rate
    lr_decays = {i: 0.95 for i in range(0, 500)}  # decay rate of learning rate
...
class DataProcessing:
    ...
    def cal_feature_dis(feature, f_neighbours):
        """
        feature: (B,N,1,d)
        f_neighbours: (B,N,k,d)
        feature_dist: [B, N, k]
        Calculate the feature distance
        """
        feature_tile = tf.tile(feature, [1, 1, tf.shape(f_neighbours)[-2], 1])
        feature_dist = feature_tile - f_neighbours
        feature_dist = tf.reduce_mean(tf.abs(feature_dist), axis=-1)
...
class Plot:
    ...
    def draw_pc_sem_ins(pc_xyz, pc_sem_ins, plot_colors=None):
        ...
        for id, semins in enumerate(sem_ins_labels):
            ...
            ### bbox
            valid_ind = valid_ind[valid_ind < len(pc_xyz)]
